...
export const getCancellation = async (cancellation: number, language: string, priceChangeRate: number) => {
  const en = bookcarsHelper.isFrench(language)

  if (cancellation === -1) {
    return `${i18n.t('CANCELLATION')}${en ? ' : ' : ': '}${i18n.t('UNAVAILABLE')}`
  } if (cancellation === 0) {
    return `${i18n.t('CANCELLATION')}${en ? ' : ' : ': '}${i18n.t('INCLUDED')}${en ? 'e' : ''}`
  }
...
export const getAmendments = async (amendments: number, language: string, priceChangeRate: number) => {
  ...
  if (amendments === -1) {
    return `${i18n.t('AMENDMENTS')}${en ? ' : ' : ': '}${i18n.t('UNAVAILABLE')}${en ? 's' : ''}`
  } if (amendments === 0) {
    return `${i18n.t('AMENDMENTS')}${en ? ' : ' : ': '}${i18n.t('INCLUDED')}${en ? 'es' : ''}`
  }
  let _amendments = await StripeService.convertPrice(amendments)
  _amendments += _amendments * (priceChangeRate / 100)
  return `${i18n.t('AMENDMENTS')}${en ? ' : ' : ': '}${bookcarsHelper.formatPrice(_amendments, (await StripeService.getCurrencySymbol()), language)}`
}
...
export const getTheftProtection = async (theftProtection: number, language: string, priceChangeRate: number) => {
  ...
  if (theftProtection === -1) {
    return `${i18n.t('THEFT_PROTECTION')}${en ? ' : ' : ': '}${i18n.t('UNAVAILABLE')}`
  } if (theftProtection === 0) {
    return `${i18n.t('THEFT_PROTECTION')}${en ? ' : ' : ': '}${i18n.t('INCLUDED')}${en ? 'e' : ''}`
  }
  ...
  return `${i18n.t('THEFT_PROTECTION')}${en ? ' : ' : ': '}${bookcarsHelper.formatPrice(_theftProtection, (await StripeService.getCurrencySymbol()), language)}${i18n.t('DAILY')}`
}
...
export const getCollisionDamageWaiver = async (collisionDamageWaiver: number, language: string, priceChangeRate: number) => {
  ...
  if (collisionDamageWaiver === -1) {
    return `${i18n.t('COLLISION_DAMAGE_WAVER')}${en ? ' : ' : ': '}${i18n.t('UNAVAILABLE')}`
  } if (collisionDamageWaiver === 0) {
    return `${i18n.t('COLLISION_DAMAGE_WAVER')}${en ? ' : ' : ': '}${i18n.t('INCLUDED')}${en ? 'e' : ''}`
  }
  ...
  return `${i18n.t('COLLISION_DAMAGE_WAVER')}${en ? ' : ' : ': '}${bookcarsHelper.formatPrice(_collisionDamageWaiver, (await StripeService.getCurrencySymbol()), language)}${i18n.t('DAILY')}`
}
...
export const getFullInsurance = async (fullInsurance: number, language: string, priceChangeRate: number) => {
  ...
  if (fullInsurance === -1) {
    return `${i18n.t('FULL_INSURANCE')}${en ? ' : ' : ': '}${i18n.t('UNAVAILABLE')}`
  } if (fullInsurance === 0) {
    return `${i18n.t('FULL_INSURANCE')}${en ? ' : ' : ': '}${i18n.t('INCLUDED')}${en ? 'e' : ''}`
  }
  let _fullInsurance = await StripeService.convertPrice(fullInsurance)
  _fullInsurance += _fullInsurance * (priceChangeRate / 100)
  return `${i18n.t('FULL_INSURANCE')}${en ? ' : ' : ': '}${bookcarsHelper.formatPrice(_fullInsurance, (await StripeService.getCurrencySymbol()), language)}${i18n.t('DAILY')}`
}
...
export const getAdditionalDriver = async (additionalDriver: number, language: string, priceChangeRate: number) => {
  ...
  if (additionalDriver === -1) {
    return `${i18n.t('ADDITIONAL_DRIVER')}${en ? ' : ' : ': '}${i18n.t('UNAVAILABLE')}`
  } if (additionalDriver === 0) {
    return `${i18n.t('ADDITIONAL_DRIVER')}${en ? ' : ' : ': '}${i18n.t('INCLUDED')}`
  }
  ...
  return `${i18n.t('ADDITIONAL_DRIVER')}${en ? ' : ' : ': '}${bookcarsHelper.formatPrice(_additionalDriver, (await StripeService.getCurrencySymbol()), language)}${i18n.t('DAILY')}`
}
...
export const getAmendmentsOption = async (amendments: number, language: string, priceChangeRate: number, hidePlus?: boolean) => {
  ...
  if (amendments === -1) {
    return `${i18n.t('UNAVAILABLE')}${en ? 's' : ''}`
  } if (amendments === 0) {
    return `${i18n.t('INCLUDED')}${en ? 'es' : ''}`
  }
  let _amendments = await StripeService.convertPrice(amendments)
  _amendments += _amendments * (priceChangeRate / 100)
